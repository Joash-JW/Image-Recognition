...
def run(frame, graph, model, count):
    ...
    if len(boxes) > 0:
        for box in boxes:
            ...
            resized = cv2.resize(target, dsize=(model.dim, model.dim), interpolation=cv2.INTER_CUBIC) # resize to match CNN input size
    ...
    cv2.putText(frame, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2) # write the class label of the image
